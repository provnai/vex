...
use algoswitch::{sort, detect_pattern};
...
fn main() {
    ...
    println!("{:25} {:12.2} µs", "quicksort", start.elapsed().as_nanos() as f64 / 1000.0);
    
    let mut d = data.clone();
    let start = Instant::now();
    sort::mergesort(&mut d);
    println!("{:25} {:12.2} µs", "mergesort", start.elapsed().as_nanos() as f64 / 1000.0);
    
    let mut d = data.clone();
    let start = Instant::now();
    sort::heapsort(&mut d);
    println!("{:25} {:12.2} µs", "heapsort", start.elapsed().as_nanos() as f64 / 1000.0);
    
    let mut d = data.clone();
    let start = Instant::now();
    sort::insertionsort(&mut d);
    println!("{:25} {:12.2} µs", "insertionsort", start.elapsed().as_nanos() as f64 / 1000.0);
    
    let mut d = data.clone();
    let start = Instant::now();
    sort::radixsort(&mut d);
    println!("{:25} {:12.2} µs", "radixsort", start.elapsed().as_nanos() as f64 / 1000.0);
    ...
    let start = Instant::now();
    sort::insertionsort(&mut d);
    println!("{:25} {:12.2} µs", "insertionsort (WINNER!)", start.elapsed().as_nanos() as f64 / 1000.0);
    ...
    let start = Instant::now();
    let pattern = detect_pattern(&data);
    println!("{:25} {:12.2} µs", "detect sorted (100K)", start.elapsed().as_nanos() as f64 / 1000.0);
    println!("Detected: {:?}", pattern);
