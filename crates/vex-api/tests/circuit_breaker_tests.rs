...
async fn test_circuit_trips_on_failures() {
    ...
    assert!(!breaker.allow().await, "Circuit should be open after 3 failures");
}
...
async fn test_circuit_recovers_after_timeout() {
    ...
    breaker.record_failure().await;
    assert!(breaker.allow().await, "Should handle 1 failure without tripping (threshold 1, but count resets)");
}
...
async fn test_half_open_failure_reopens() {
    ...
    assert!(!breaker.allow().await, "Should re-open after failure in HalfOpen");
}
