...
use vex_queue::{WorkerPool, WorkerConfig, QueueBackend};
use vex_llm::{RateLimiter, RateLimitConfig, Metrics};
use std::sync::Arc;
...
async fn test_full_lifecycle() {
    ...
    assert!(execute_res["response"].as_str().unwrap().starts_with("Job queued"));
