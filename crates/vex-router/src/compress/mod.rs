...
impl PromptCompressor {
    ...
    fn extract_key_information(&self, text: &str) -> String {
        ...
        let important_markers = [
            "important", "critical", "key", "must", "require", "need", "task", "goal",
            "create", "build", "make", "write", "find", "get", "calculate", "solve",
        ];
        ...
        if important_sentences.is_empty() {
            important_sentences.iter().take(2).copied().collect::<Vec<_>>().join(". ")
        } else {
