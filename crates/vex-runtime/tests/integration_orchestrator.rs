...
async fn test_orchestrator_chora_gate_flow() {
    ...
    let result = orchestrator.process("test-tenant", prompt, vec![]).await.expect("Execution failed");
    ...
    assert_eq!(capsule.outcome, "ALLOW", "Gate should have allowed execution");
    assert!(!capsule.witness_receipt.is_empty(), "Witness receipt should be present");
