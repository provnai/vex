...
pub fn create_identity_provider(allow_fallback: bool) -> Result<Box<dyn HardwareIdentity>, HardwareError> {
    #[cfg(windows)]
    {
        if allow_fallback {
            return Ok(Box::new(stub_impl::StubIdentity::default()));
        }
        return Ok(Box::new(windows_impl::CngIdentity::default()));
    }
    ...
    #[cfg(not(any(windows, target_os = "linux")))]
    {
        if allow_fallback {
            ...
            Err(HardwareError::NoTpmFound("Hardware identity not supported on this platform".to_string()))
        }
    }
}
    ...
    impl HardwareIdentity for CngIdentity {
        async fn seal(&self, _label: &str, data: &[u8]) -> Result<Vec<u8>> {
             unsafe {
                ...
                if status != 0 {
                    return Err(anyhow!("TPM provider not available (Status: 0x{:X})", status));
                }
                ...
                if status != 0 {
                    ...
                    if status != 0 {
                        ...
                        NCryptFreeObject(provider);
                        return Err(anyhow!("Failed to finalize TPM key (Status: 0x{:X})", status));
                    }
                }
                ...
                if status != 0 {
                    NCryptFreeObject(key_handle);
                    NCryptFreeObject(provider);
                    return Err(anyhow!("Failed to get ciphertext size (Status: 0x{:X})", status));
                }
                ...
                if status != 0 {
                    return Err(anyhow!("Failed to encrypt with TPM (Status: 0x{:X})", status));
                }
        ...
        async fn unseal(&self, blob: &[u8]) -> Result<Vec<u8>> {
             unsafe {
                ...
                if status != 0 {
                    NCryptFreeObject(provider);
                    return Err(anyhow!("Identity key not found in TPM (Status: 0x{:X})", status));
                }
                ...
                if status != 0 {
                    NCryptFreeObject(key_handle);
                    NCryptFreeObject(provider);
                    return Err(anyhow!("Failed to get decrypted size (Status: 0x{:X})", status));
                }
                ...
                if status != 0 {
                    return Err(anyhow!("Failed to unseal with TPM (Status: 0x{:X})", status));
                }
...
mod linux_impl {
    ...
    use tss_esapi::{
        ...
        structures::{
            PublicBuilder, PublicRsaParametersBuilder,
            PublicKeyedHashParameters, KeyedHashScheme,
            SymmetricDefinitionObject, RsaExponent, Public,
            Auth, SensitiveData, Private, PublicBuffer,
        },
        utils,
    ...
    impl Tpm2Identity {
        pub fn new() -> Result<Self> {
            ...
            let context = Context::new(tcti).map_err(|e| anyhow!("Failed to create TPM context: {}", e))?;
            Ok(Self {
    ...
    impl HardwareIdentity for Tpm2Identity {
        async fn seal(&self, _label: &str, data: &[u8]) -> Result<Vec<u8>> {
            ...
            tokio::task::spawn_blocking(move || {
                let mut context = context_lock.lock().map_err(|e| anyhow!("Mutex error: {}", e))?;
                ...
                let object_attributes = ObjectAttributesBuilder::new()
                    ...
                    .build().map_err(|e| anyhow!("Failed to build object attributes: {}", e))?;
                ...
                let sealed_data_public = PublicBuilder::new()
                    ...
                    .build().map_err(|e| anyhow!("Failed to build sealed data public structure: {}", e))?;
                ...
                let pub_buf = PublicBuffer::try_from(public)
                    ...
                    .marshall().map_err(|e| anyhow!("Pub marshall error: {}", e))?;
        ...
        async fn unseal(&self, blob: &[u8]) -> Result<Vec<u8>> {
            ...
            tokio::task::spawn_blocking(move || {
                ...
                if blob.len() < 4 { return Err(anyhow!("Invalid blob length")); }
                let pub_len = u32::from_le_bytes(blob[0..4].try_into().unwrap()) as usize;
                if blob.len() < 4 + pub_len { return Err(anyhow!("Invalid pub length in blob")); }
                ...
                let object_handle = context.load(primary_key_handle, private, public)
                    .map_err(|e| anyhow!("Failed to load sealed object: {}", e))?;

                // 4. Unseal
                let unsealed_data = context.unseal(object_handle.into())
...
mod tests {
    ...
    async fn test_hardware_identity_interface() {
        let provider = create_identity_provider();
        let data = b"test_secret_seed";
        
        match provider.seal("test_label", data).await {
            Ok(sealed) => {
                let unsealed = provider.unseal(&sealed).await.expect("Unseal must succeed if seal succeeded");
                assert_eq!(data.to_vec(), unsealed, "Unsealed data must match original input");
            }
            Err(e) => {
