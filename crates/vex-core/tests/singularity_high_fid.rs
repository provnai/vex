...
async fn test_singularity_high_fidelity_tpm() {
    ...
    let sealed_seed = temp_tpm
        .seal("identity_v1", &master_seed)
        .await
        .expect("Failed to seal master seed to TPM hardware");
    println!(
        "✅ Master Seed sealed to TPM. Blob size: {} bytes",
        sealed_seed.len()
    );
    ...
    let kp = Arc::new(
        TpmKeyProvider::new(Some(sealed_seed), Some(verifying_key_bytes.to_vec()))
            .expect("Failed to initialize TpmKeyProvider with sealed seed"),
    );

    let aid = kp.aid().await.unwrap();
    println!(
        "✅ Attest-RS TPM Provider Active. AID: {}",
        hex::encode(aid)
    );
    ...
    let is_valid = packet
        .verify(&public_key_result)
        .expect("Verification logic error");
    ...
    println!(
        "✅ Capsule Root Integrity Confirmed: {}",
        reconstructed_root.to_hex()
    );
