...
impl EvolutionMemory {
    ...
    pub fn get_experiments_snapshot(&self) -> Vec<GenomeExperiment> {
        self.experiments.iter().map(|(exp, _)| exp.clone()).collect()
    }

    /// Get snapshot of the oldest N experiments
    pub fn get_experiments_oldest(&self, count: usize) -> Vec<GenomeExperiment> {
        self.experiments.iter().take(count).map(|(exp, _)| exp.clone()).collect()
    }
    ...
    fn update_correlations(&mut self) {
        ...
        let trait_count = self.experiments.front()
            .map(|(e, _)| e.traits.len())
            .unwrap_or(5);

        for i in 0..trait_count {
            let trait_name = self.experiments.front()
                .and_then(|(e, _)| e.trait_names.get(i).cloned())
                .unwrap_or_else(|| format!("trait_{}", i));

            let trait_values: Vec<f64> = self.experiments.iter()
                .filter_map(|(e, _)| e.traits.get(i).copied())
                .collect();
            let fitness_values: Vec<f64> = self.experiments.iter()
...
mod tests {
    ...
    fn test_pearson_correlation() {
        ...
        assert!((corr_neg + 1.0).abs() < 0.001, "Expected ~-1.0, got {}", corr_neg);

        // No correlation (random)
        let y_rand = vec![3.0, 1.0, 4.0, 2.0, 5.0];
        let corr_rand = pearson_correlation(&x, &y_rand);
        assert!(corr_rand.abs() < 0.8, "Expected low correlation, got {}", corr_rand);
    }
    ...
    fn test_correlation_learning() {
        ...
        let corr = memory.correlations().get("exploration").copied().unwrap_or(0.0);
    ...
    fn test_eviction() {
        ...
        let genome = Genome::new("Test");
        for i in 0..10 {
            let exp = GenomeExperiment::new(&genome, HashMap::new(), i as f64 / 10.0, &format!("Task {}", i));
            memory.record(exp);
        }
    ...
    fn test_suggest_adjustments() {
        ...
        assert!(exp_suggestion.is_some(), "Should suggest exploration adjustment");
        if let Some(s) = exp_suggestion {
            assert!(s.suggested_value > s.current_value, "Should suggest increasing exploration");
        }
    }
    ...
    fn test_dos_memory_bounded() {
        ...
        assert!(memory.len() <= 500, "Memory grew unbounded: {} entries", memory.len());
        
        // High-fitness experiments should be kept
        let top = memory.get_top_experiments(10);
        assert!(top.iter().all(|e| e.overall_fitness >= 0.1), "Lost high-fitness experiments");
    }
    ...
    fn test_capacity_limit() {
        ...
        assert!(memory.max_entries <= 10_000, "Capacity not capped: {}", memory.max_entries);
    }
}
